...
|---|---|---|
| `fix:` | patch (1.5.1 → 1.5.2) | yes, "Bug Fixes" |
| `feat:` | minor (1.5.1 → 1.6.0) | yes, "Features" |
| `feat!:` or `BREAKING CHANGE:` footer | major (1.5.1 → 2.0.0) | yes, highlighted |
...
Editor sources are the exception to the "outside `public/`" rule: `src/` is compiled
into `public/dist/` and does reach users, so a change there can be a `fix:` or `feat:`
...
| `public/dist/` | compiled editor assets — **not in the repository**, built from `src/` |
...
`public/ph-postqueue.php` and the `Stable tag:` in `public/readme.txt` are all
