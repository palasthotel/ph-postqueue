...
<div class="postqueue-metabox-wrapper">
  ...
  <div class="postqueue-metabox-postqueuelist-wrapper">
    ...
    <ul>
      ...
      <?php $postqueue = $postqueue[0]; ?>
        <li><?php echo esc_html( $postqueue->name ); ?> <span class="dashicons dashicons-no postqueue-remove" data-queueid="<?php echo esc_attr( $postqueue->queue_id ); ?>" data-postid="<?php echo esc_attr( get_the_ID() ); ?>" title="<?php esc_attr_e( 'Remove post from this postqueue.', Postqueue\Plugin::DOMAIN ); ?>" data-queuename="<?php echo esc_attr( $postqueue->name ); ?>"></span></li>
      <?php endforeach; ?>
    </ul>
  </div>
  ...
  <div class="postqueue-metabox-postqueueselect-wrapper">
    ...
    <?php endif; ?>
    <span class="button button-secondary hide-if-no-js postqueue-add" data-postid="<?php echo esc_attr( get_the_ID() ); ?>"><?php esc_html_e( 'Add', Postqueue\Plugin::DOMAIN ); ?></span>
  </div>
</div>